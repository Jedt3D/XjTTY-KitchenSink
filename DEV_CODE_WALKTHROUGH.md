...
XjTTY-KitchenSink is a **pure consumer** of the `XjTTYLib` library. The app has 5 source files:

```
KSApp.xojo_code              ← Entry point. Owns everything.
KSComponentRegistry.xojo_code ← Data: what components exist and their metadata
KSPreviewBuilder.xojo_code   ← Factory: builds UI widget trees for each component
KSStatusBar.xojo_code        ← Renders the bottom status line
KitchenSink.xojo_project     ← Project file (references all the above + XjTTYLib/)
```

**The rule:** `KSApp` orchestrates. `KSComponentRegistry` owns data. `KSPreviewBuilder` builds UI. `KSStatusBar` renders one line. Each file knows its job and nothing else.
